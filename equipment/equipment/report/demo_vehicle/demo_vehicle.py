...
def get_vehicle_log_data(filters):
	...
	data = frappe.db.sql(
		...
		SELECT
			...
			vhcl.location, vhcl.project, log.name as log_name, log.odometer,
...
def get_conditions(filters):
	...
	if filters.project:
		conditions += " and vhcl.project = %(project)s"
...
def get_chart_data(data, filters):
	...
	expense = [fuel_exp_data, service_exp_data]
	vehicle_expense = [sum(x) for x in zip(*expense)]
	...
	if vehicle_expense:
		datasets.append({"name": _("Vehicle Expenses"), "values": vehicle_expense})
